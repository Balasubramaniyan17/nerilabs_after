...
(function() {
    ...
    // Handle ?neri_reset or ?neri_new to allow founders to test new random visitor assignments
    if (urlParams.get("neri_reset") || urlParams.get("neri_new")) {
        try {
            localStorage.removeItem("neri_vid");
            sessionStorage.removeItem("neri_sid");
            sessionStorage.removeItem("neri_active_preview");
            document.cookie = "neri_vid=;path=/;expires=Thu, 01 Jan 1970 00:00:01 GMT;";
            console.info("[NeriLabs Universal SDK] Reset visitor identity for fresh assignment testing.");
        } catch(e) {}
    }

    let previewParam = urlParams.get("neri_preview") || urlParams.get("neri_variant");
    if (previewParam) {
        sessionStorage.setItem("neri_active_preview", previewParam);
    } else {
        previewParam = sessionStorage.getItem("neri_active_preview") || null;
    }
...
// Event Ingestion Queue
    ...
    function handlePromotionalRescueAction(action, newToken) {
        ...
        if (action.discounted_price_formatted) {
            ...
            if (targetAssign) {
                ...
                // Prioritize action.target_selector so exact configured tier is discounted
                const pSel = action.target_selector || (targetAssign.payload && targetAssign.payload.pricing_payload && targetAssign.payload.pricing_payload.price_selector) || ".pricing-grid > :nth-child(2) .plan-price, .pricing-grid > :nth-child(2) .price-amount";
                let targetEls = Array.from(document.querySelectorAll(pSel)).filter(m => !isButtonOrLink(m));
                
                // Fallback to matching card by tier title (e.g. Starter, Growth, Scale)
                if (targetEls.length === 0 && action.plan_name) {
                    const cardHeaders = Array.from(document.querySelectorAll(".plan-name, .price-name, h2, h3, h4, [class*='tier-name'], [class*='plan-title']"));
                    const targetTierKey = action.plan_name.toLowerCase();
                    for (const header of cardHeaders) {
                        const hText = (header.innerText || header.textContent || "").trim().toLowerCase();
                        if (hText && (targetTierKey.includes(hText) || hText.includes(targetTierKey.split(" ")[0]))) {
                            const card = header.closest(".price-card, .card, [class*='hud-card'], [class*='tier']");
                            if (card) {
                                const pEl = card.querySelector(".plan-price, .price-amount, .amount, .cost, .price");
                                if (pEl) {
                                    targetEls = [pEl];
                                    break;
                                }
    ...
    // Monitor live dwell hesitation & exit intent with active pricing hover tracking
    function setupBehavioralFrictionMonitors() {
        let dwellSeconds = 0;
        let pricingHoverSeconds = 0;
        let isHoveringPricing = false;

        // 1. Actively track hover dwell over pricing table/cards
        const pricingSelectors = ["#pricing", ".pricing-grid", ".price-card", "[class*='pricing']", "[class*='plan']"];
        try {
            const pricingElements = document.querySelectorAll(pricingSelectors.join(","));
            pricingElements.forEach(el => {
                el.addEventListener("mouseenter", () => {
                    isHoveringPricing = true;
                    console.info("[NeriLabs Rescue] Cursor entered pricing cards. Tracking hesitation dwell...");
                });
                el.addEventListener("mouseleave", () => {
                    isHoveringPricing = false;
                });
            });
        } catch(e) {}

        // Track cursor movement across pricing section and cards dynamically
        document.addEventListener("mousemove", function(e) {
            const target = e.target;
            if (target && target.closest && target.closest("#pricing, .pricing-grid, .price-card, [class*='pricing'], [class*='plan']")) {
                isHoveringPricing = true;
            }
        }, { passive: true });
        ...
        const dwellInterval = setInterval(() => {
            dwellSeconds += 1;
            if (isHoveringPricing) pricingHoverSeconds += 1;

            // Trigger when pricing hover reaches 4s, or 7s with hover, or 10s page dwell
            const shouldTrigger = (pricingHoverSeconds >= 4) || (dwellSeconds >= 7 && isHoveringPricing) || (dwellSeconds >= 10);
            if (shouldTrigger && activeAssignments.length > 0 && !rescueModalRendered) {
                clearInterval(dwellInterval);
                console.info(`[NeriLabs Rescue] Dwell hesitation detected (${pricingHoverSeconds}s pricing hover, ${dwellSeconds}s page dwell). Dispatching promotional rescue request...`);
                queueEvent(
                    activeAssignments[0].experiment_id,
                    activeAssignments[0].signed_token,
                    activeAssignments[0].opaque_variant_id,
                    "DWELL",
                    { dwell_duration: Math.max(pricingHoverSeconds, dwellSeconds), hovered_element: isHoveringPricing ? "Pricing Card" : "Pricing Section" },
        ...
        document.addEventListener("mouseleave", function(e) {
            if (e.clientY <= 0 && activeAssignments.length > 0 && !rescueModalRendered) {
                console.info("[NeriLabs Rescue] Exit intent detected. Dispatching promotional concession request...");
                queueEvent(
                    activeAssignments[0].experiment_id,
                    activeAssignments[0].signed_token,
                    activeAssignments[0].opaque_variant_id,
                    "DWELL",
                    { is_exit_intent: true, dwell_duration: Math.max(pricingHoverSeconds, dwellSeconds) },
                    activeAssignments[0].tenant_id
                );
                flushBeacon();
            }
...
function flushBeacon() {
        ...
        console.info(`[NeriLabs Telemetry] Flushing beacon (${batch.length} event(s)) to ${endpoint}...`);
        fetch(endpoint, {
            method: "POST",
            headers: {
    ...
    // Expose global API for programmatic conversions (e.g. on custom checkout success callbacks)
    window.NERILABS = window.NERILABS || {};
    window.NERILABS.convert = function(customData) {
        if (activeAssignments && activeAssignments.length > 0) {
            activeAssignments.forEach(assign => {
                queueEvent(
                    assign.experiment_id,
                    assign.signed_token,
                    assign.opaque_variant_id,
                    "CONVERSION",
                    Object.assign({ source: "programmatic_api" }, customData || {}),
                    assign.tenant_id
                );
            });
            flushBeacon();
            console.info("[NeriLabs Telemetry] Programmatic conversion recorded successfully.");
        }
    };

    // Automatic Post-Checkout Detection (Success / Thank-You URLs):
    function checkPostCheckoutSuccess() {
        ...
        const isSuccessPage = (
            ...
            search.includes("checkout_success=true")
        );
        if (isSuccessPage) {
            const alreadyRecorded = sessionStorage.getItem("neri_purchase_completed");
            if (!alreadyRecorded && activeAssignments && activeAssignments.length > 0) {
                sessionStorage.setItem("neri_purchase_completed", "true");
                activeAssignments.forEach(assign => {
                    queueEvent(
                        assign.experiment_id,
                        assign.signed_token,
                ...
                flushBeacon();
                console.info("[NeriLabs Telemetry] Verified post-checkout conversion recorded from success page.");
            }
        }
    }
    ...
    function applySingleAssignment(assignment, el) {
        ...
        if (assignment.client_action === "forced_preview" && previewParam !== "control") {
            assignment.is_control = false;
        }
        ...
        if (assignment.is_control) {
            ...
            if (payload.pricing_payload) {
                ...
                // Plan Name Disambiguation: If selector did not match, find card by plan name header
                if (priceElements.length === 0) {
                    const targetName = (pp.plan_name || assignment.variant_name || "").toLowerCase();
                    const cardHeaders = Array.from(document.querySelectorAll(".plan-name, .price-name, h2, h3, h4, [class*='tier-name'], [class*='plan-title']"));
                    for (const header of cardHeaders) {
                        const hText = (header.innerText || header.textContent || "").trim().toLowerCase();
                        if (hText && (targetName.includes(hText) || hText.includes(targetName.split(" ")[0]))) {
                            const card = header.closest(".price-card, .card, [class*='hud-card'], [class*='price-box'], [class*='tier']");
                            if (card) {
                                const priceInCard = card.querySelector(".plan-price, .price-amount, .amount, .cost, .price");
                                if (priceInCard) {
                                    priceElements = [priceInCard];
                                    break;
                                }
                            }
                        }
                ...
                // Disambiguate if multiple cards matched across the pricing table
                if (priceElements.length > 1) {
                    const targetName = (pp.plan_name || assignment.variant_name || "").toLowerCase();
                    const filtered = priceElements.filter(pEl => {
                        const card = pEl.closest(".price-card, .card, [class*='hud-card'], [class*='price-box'], [class*='tier'], [class*='plan']");
                        if (!card) return false;
                        const cText = (card.innerText || card.textContent || "").toLowerCase();
                        const planKeywords = ["starter", "growth", "scale", "launch", "enterprise", "pro", "basic", "premium", "team", "business", "small", "medium", "large"];
                        for (const kw of planKeywords) {
                            if (targetName.includes(kw) && cText.includes(kw)) return true;
                        }
                        return false;
                    });
                    if (filtered.length > 0) {
                        priceElements = filtered;
                    }
                }
                ...
                if (formatted && priceElements.length > 0) {
                    priceElements.forEach(pEl => {
                        ...
                        // Avoid stomping by another concurrent experiment on a different tier
                        const existingExp = pEl.getAttribute("data-neri-exp");
                        if (existingExp && existingExp !== assignment.experiment_id) return;
                        pEl.setAttribute("data-neri-exp", assignment.experiment_id);
    ...
    function init() {
        ...
        // SPA Route Navigation Handling:
        // Automatically re-applies mutations when visitors navigate across pages (/pricing, /features)
        function handleSPARouteChange() {
            console.info("[NeriLabs Universal SDK] Route change detected. Re-evaluating page mutations...");
            if (activeAssignments && activeAssignments.length > 0) {
                activeAssignments.forEach(assign => observeAndApply(assign, 6000));
            }
            setupBehavioralFrictionMonitors();
        }
        ...
        if (rawPushState) {
            history.pushState = function() {
                ...
                setTimeout(handleSPARouteChange, 120);
            };
        }
        ...
        window.addEventListener("popstate", () => setTimeout(handleSPARouteChange, 120));
        window.addEventListener("hashchange", () => setTimeout(handleSPARouteChange, 120));
    }
