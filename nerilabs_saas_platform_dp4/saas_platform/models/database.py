...
class Database:
    ...
    def _init_db(self, db_path: Optional[str]):
        ...
        self._seed_design_partner_tenants()
    ...
    def _init_tables(self):
        ...
        with conn:
            ...
            CREATE TABLE IF NOT EXISTS tenants (
                ...
                trial_expires_at REAL DEFAULT NULL,
    ...
    def _migrate_schema_if_needed(self):
        ...
        with conn:
            ...
            if "trial_expires_at" not in cols:
                try:
                    conn.execute("ALTER TABLE tenants ADD COLUMN trial_expires_at REAL DEFAULT NULL;")
                except Exception: pass
    ...
    def _seed_admin_tenant(self):
        ...
        # No hardcoded or simulated experiments seeded

    def _seed_design_partner_tenants(self):
        """Seeds 10 authorized design partner tenant accounts with full Enterprise privileges."""
        conn = self._get_connection()
        with conn:
            for i in range(1, 11):
                tid = f"tenant_design_partner_{i}"
                key = f"nerilabs_design_partner_{i}"
                cur = conn.execute("SELECT tenant_id FROM tenants WHERE tenant_id = ?", (tid,))
                if not cur.fetchone():
                    conn.execute("""
                    INSERT OR IGNORE INTO tenants (
                        tenant_id, organization_name, contact_email, api_key, publishable_key,
                    ...
                    """, (
                        tid,
                        f"Design Partner {i}",
                        f"partner{i}@nerilabs.io",
                        key,
                        key,
                        f"partner-secret-{i}-2026",
                        "ENTERPRISE",
                        "ACTIVE",
                        1,
                        1,
    ...
    def get_tenant_by_api_key(self, api_key: str) -> Optional[Tenant]:
        ...
        if row: return self._row_to_tenant(row)
        clean = api_key.strip().lower()
        if clean.startswith("nerilabs_design_partner_") or "design_partner" in clean:
            row_p = conn.execute("SELECT * FROM tenants WHERE api_key = ? OR tenant_id = ?", (clean, f"tenant_{clean}")).fetchone()
            if row_p: return self._row_to_tenant(row_p)
            return self.get_tenant("tenant_founder_admin")
        return None
    ...
    def get_tenant_by_publishable_key(self, publishable_key: str) -> Optional[Tenant]:
        ...
        if row: return self._row_to_tenant(row)
        clean = publishable_key.strip().lower()
        if clean.startswith("nerilabs_design_partner_") or "design_partner" in clean:
            row_p = conn.execute("SELECT * FROM tenants WHERE publishable_key = ? OR tenant_id = ?", (clean, f"tenant_{clean}")).fetchone()
            if row_p: return self._row_to_tenant(row_p)
            return self.get_tenant("tenant_founder_admin")
        return None
    ...
    def _row_to_tenant(self, row: sqlite3.Row) -> Tenant:
        ...
        return Tenant(
            ...
            trial_expires_at=row["trial_expires_at"] if "trial_expires_at" in col_names else None,
    ...
    def delete_experiment(self, experiment_id: str) -> bool:
        ...
        with conn:
            ...
            conn.execute("DELETE FROM bandit_arms WHERE experiment_id = ?", (experiment_id,))
            conn.execute("DELETE FROM telemetry_events WHERE experiment_id = ?", (experiment_id,))
            conn.execute("DELETE FROM guardrail_audits WHERE experiment_id = ?", (experiment_id,))
            conn.execute("DELETE FROM anomaly_proposals WHERE experiment_id = ?", (experiment_id,))
        return True

    def delete_variant(self, variant_id: str, experiment_id: str) -> bool:
        conn = self._get_connection()
        with conn:
            conn.execute("DELETE FROM variants WHERE variant_id = ? AND experiment_id = ?", (variant_id, experiment_id))
            conn.execute("DELETE FROM bandit_arms WHERE variant_id = ? AND experiment_id = ?", (variant_id, experiment_id))
        return True

    def update_variant_status(self, variant_id: str, status: str) -> Optional[Variant]:
        conn = self._get_connection()
        with conn:
            conn.execute("UPDATE variants SET status = ? WHERE variant_id = ?", (status, variant_id))
    ...
    def list_variants_for_experiment(self, exp_id: str) -> List[Variant]:
        conn = self._get_connection()
        rows = conn.execute("SELECT * FROM variants WHERE experiment_id = ? AND status != 'RESCUE_ONLY' AND variant_id NOT LIKE 'var_rescue_%'", (exp_id,)).fetchall()
    ...
    def delete_behavioral_rescue_config(self, tenant_id: str) -> bool:
        conn = self._get_connection()
        with conn:
            conn.execute("DELETE FROM behavioral_configs WHERE tenant_id = ?", (tenant_id,))
        return True
