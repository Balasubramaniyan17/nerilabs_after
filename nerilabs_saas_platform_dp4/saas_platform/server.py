...
def is_admin_key(key: Optional[str]) -> bool:
    ...
    k = key.strip().lower()
    return (
        k == Config.ADMIN_MASTER_KEY.lower() or
        ...
        k.startswith("nerilabs_master_") or
        k.startswith("nerilabs_design_partner_") or
        k in Config.DESIGN_PARTNER_KEYS or
        "design_partner" in k
    )


def check_partner_lock_status(tenant: Tenant):
    if tenant.subscription_status in ("LOCKED", "SUSPENDED", "TRIAL_EXPIRED"):
        raise HTTPException(
            status_code=403,
            detail=f"Access Denied: Design partner access is locked (Status: {tenant.subscription_status}). Please contact founder@nerilabs.io."
        )
    if tenant.trial_expires_at and time.time() > tenant.trial_expires_at:
        tenant.subscription_status = "TRIAL_EXPIRED"
        db.update_tenant(tenant)
        raise HTTPException(
            status_code=403,
            detail="Access Denied: Design partner trial has expired. Key has been locked."
        )
...
def authenticate_tenant(x_api_key: Optional[str]) -> Tenant:
    ...
    if is_admin_key(clean_key):
        if "design_partner" in clean_key.lower():
            partner_t = db.get_tenant_by_api_key(clean_key)
            if partner_t:
                check_partner_lock_status(partner_t)
                partner_t.subscription_plan = "ENTERPRISE"
                partner_t.plan_limits = PLAN_LIMITS["ENTERPRISE"]
                return partner_t
...
def is_founder_admin_key(key: Optional[str]) -> bool:
    if not key:
        return False
    k = key.strip().lower()
    return (
...
def verify_founder_admin(x_api_key: Optional[str] = Header(None, alias="X-API-Key")) -> Tenant:
    tenant = authenticate_tenant(x_api_key)
    clean_key = (x_api_key or "").strip().lower()
    if not is_founder_admin_key(clean_key) and tenant.tenant_id != "tenant_founder_admin":
        raise HTTPException(
            status_code=403,
            detail="Forbidden: Founder Master Admin authorization required."
        )
    return tenant
...
def sync_auth0_user(req: Auth0SyncReq):
    ...
    if not tenant:
        ...
        tenant = db.create_tenant(
            ...
            auth0_sub=req.sub,
            subscription_plan="LAUNCH"  # All free signups default strictly to LAUNCH. Upgrades require completed Stripe checkout.
        )
...
def login_with_admin_key(req: AdminLoginReq):
    key_clean = req.admin_key.strip()
    if not is_admin_key(key_clean):
        raise HTTPException(status_code=401, detail="Invalid Founder Admin Master Key or Design Partner Key")

    # Dedicated Design Partner login
    if "design_partner" in key_clean.lower():
        partner_t = db.get_tenant_by_api_key(key_clean)
        if not partner_t:
            db._seed_design_partner_tenants()
            partner_t = db.get_tenant_by_api_key(key_clean)
        if partner_t:
            check_partner_lock_status(partner_t)
            return {
                "status": "success",
                "is_admin": True,
                "tenant_id": partner_t.tenant_id,
                "organization_name": partner_t.organization_name,
                "api_key": partner_t.api_key,
                "publishable_key": partner_t.publishable_key,
                "subscription_plan": "ENTERPRISE",
                "plan_limits": PLAN_LIMITS["ENTERPRISE"]
            }
...
def get_current_tenant(x_api_key: Optional[str] = Header(None, alias="X-API-Key")):
    tenant = authenticate_tenant(x_api_key)
    clean_key = (x_api_key or "").strip().lower()
    founder = is_founder_admin_key(clean_key) or tenant.tenant_id == "tenant_founder_admin"
    is_admin = founder or (tenant.subscription_plan == "ENTERPRISE" or is_admin_key(tenant.api_key) or is_admin_key(clean_key))
    if is_admin:
...
def list_tenant_experiments(
    ...
    """Strict Multi-Tenant Isolation: Every tenant only ever views experiments in their own workspace."""
    tenant = authenticate_tenant(x_api_key)
...
@app.delete("/api/v1/experiments/{experiment_id}/variants/{variant_id}")
def delete_single_variant(
    experiment_id: str,
    variant_id: str,
    x_api_key: Optional[str] = Header(None, alias="X-API-Key")
):
    """Deletes an individual variant from an experiment and from the Combinations Matrix."""
    tenant = authenticate_tenant(x_api_key)
    is_admin = (tenant.tenant_id == "tenant_founder_admin" or tenant.subscription_plan == "ENTERPRISE" or is_admin_key(tenant.api_key) or is_admin_key(x_api_key))
    exp = db.get_experiment(experiment_id)
    if not exp:
        raise HTTPException(status_code=404, detail="Experiment not found")
    if exp.tenant_id != tenant.tenant_id:
    ...
    target_v = next((v for v in variants if v.variant_id == variant_id), None)
    if not target_v:
        raise HTTPException(status_code=404, detail="Variant not found")
    if target_v.is_control:
        raise HTTPException(status_code=400, detail="Cannot delete the Control (Baseline) variant.")

    db.delete_variant(variant_id, experiment_id)
    if experiment_id in MAB_ENGINES:
        MAB_ENGINES[experiment_id].arms.pop(variant_id, None)

    return {
        "status": "success",
        "message": f"Variant '{variant_id}' deleted from experiment.",
...
def ingest_telemetry_batch(
    ...
    rescue_action_data = None
    rescue_new_token = None
    recorded_count = 0
    duplicate_count = 0
    ...
    for event in req.events:
        ...
        if is_new:
            ...
            if not is_bot:
                ...
                triggered, new_token, rescue_payload, msg = FrictionEngine.evaluate_behavioral_event(event)
                if triggered and rescue_payload:
                    rescue_action_data = rescue_payload
                    rescue_new_token = new_token
        else:
            duplicate_count += 1
    ...
    return {
        ...
        "rescue_action": rescue_action_data,
        "new_token": rescue_new_token
    }
...
def create_pricing_experiment(
    ...
    # Prior pricing experiments remain active so founders can run simultaneous tests on different tiers (Starter, Growth, Scale)
...
@app.delete("/api/v1/billing/behavioral-rescue")
def delete_behavioral_rescue(
    x_api_key: Optional[str] = Header(None, alias="X-API-Key")
):
    """Deletes and disables any active behavioral rescue policy / founder discount for the tenant."""
    tenant = authenticate_tenant(x_api_key)
    db.delete_behavioral_rescue_config(tenant.tenant_id)
    return {"status": "success", "message": "Behavioral rescue policy and founder discount deleted."}
...
def create_upgrade_checkout(
    ...
    """Generates a Stripe Checkout URL using configured Stripe Price IDs or Payment Links."""
    tenant = authenticate_tenant(x_api_key)
    plan_key = req.plan.lower().strip()
    
    price_id_map = {
        "starter": Config.STRIPE_PRICE_STARTER or os.getenv("STRIPE_PRICE_STARTER", ""),
        "growth": Config.STRIPE_PRICE_GROWTH or os.getenv("STRIPE_PRICE_GROWTH", ""),
        "scale": Config.STRIPE_PRICE_SCALE or os.getenv("STRIPE_PRICE_SCALE", "")
    }
    payment_link_map = {
        "starter": Config.STRIPE_PAYMENT_LINK_STARTER or os.getenv("STRIPE_PAYMENT_LINK_STARTER", ""),
        "growth": Config.STRIPE_PAYMENT_LINK_GROWTH or os.getenv("STRIPE_PAYMENT_LINK_GROWTH", ""),
        "scale": Config.STRIPE_PAYMENT_LINK_SCALE or os.getenv("STRIPE_PAYMENT_LINK_SCALE", "")
    }
    plan_prices = {
        "starter": {"amount_cents": 4900, "name": "Starter Plan"},
        "growth": {"amount_cents": 9900, "name": "Growth Plan"},
        "scale": {"amount_cents": 19900, "name": "Scale Plan"}
    }
    ...
    # Option A: Direct Stripe Payment Link (buy.stripe.com/...)
    direct_link = payment_link_map.get(plan_key, "").strip()
    if direct_link:
        sep = "&" if "?" in direct_link else "?"
        return {
            "status": "success",
            "checkout_url": f"{direct_link}{sep}client_reference_id={tenant.tenant_id}&prefilled_email={tenant.contact_email or ''}",
            "plan": plan_key
        }

    # Option B: Stripe API Checkout Session with configured Price ID
    stripe_key = os.getenv("STRIPE_API_KEY") or Config.STRIPE_API_KEY
    price_id = price_id_map.get(plan_key, "").strip()
    ...
    if stripe_key and not stripe_key.startswith("sk_test_mock"):
        ...
        if price_id:
            ...
            params["line_items[0][price_data][product_data][name]"] = f"NeriLabs {target_plan['name']}"
            params["line_items[0][price_data][unit_amount]"] = target_plan["amount_cents"]
        ...
        if success and isinstance(resp, dict) and "url" in resp:
            return {
                "status": "success",
            ...
            err_msg = resp.get("error", {}).get("message") if isinstance(resp, dict) and "error" in resp else "Stripe checkout creation failed."
            raise HTTPException(status_code=400, detail=f"Stripe Checkout Error: {err_msg}")

    # Reject without Stripe configuration. Never grant free automatic upgrades!
    raise HTTPException(
        status_code=400,
        detail=f"Stripe is not configured yet. Please set your Stripe API Key (STRIPE_API_KEY) and Price IDs in your .env file (STRIPE_PRICE_{plan_key.upper()})."
    )
...
def confirm_upgrade(
    ...
    """Confirms checkout and upgrades tenant plan ONLY after verifying paid status with Stripe."""
    tenant = authenticate_tenant(x_api_key)
    session_id = (req.session_id or "").strip()
    if not session_id:
        raise HTTPException(status_code=400, detail="Missing checkout session_id. Verification failed.")

    stripe_key = os.getenv("STRIPE_API_KEY") or Config.STRIPE_API_KEY
    if not stripe_key or stripe_key.startswith("sk_test_mock"):
        raise HTTPException(status_code=400, detail="Stripe API is not configured. Cannot verify payment.")

    success, session_data = StripeBillingService._call_stripe_api(
        endpoint=f"/checkout/sessions/{session_id}",
        method="GET"
    )
    if not success or not session_data or not isinstance(session_data, dict):
        raise HTTPException(status_code=400, detail="Invalid Stripe checkout session. Verification failed.")

    payment_status = session_data.get("payment_status")
    status = session_data.get("status")
    client_ref = session_data.get("client_reference_id")
    meta_tenant = session_data.get("metadata", {}).get("tenant_id")

    if payment_status != "paid" and status != "complete":
        raise HTTPException(status_code=400, detail=f"Checkout session is not paid (status: {payment_status}/{status}).")

    if client_ref != tenant.tenant_id and meta_tenant != tenant.tenant_id:
        raise HTTPException(status_code=403, detail="Checkout session does not belong to the authenticated workspace.")

    plan_to_apply = (session_data.get("metadata", {}).get("plan") or req.plan).upper().strip()
    if plan_to_apply in ["STARTER", "GROWTH", "SCALE", "ENTERPRISE"]:
        tenant.subscription_plan = plan_to_apply
        tenant.subscription_status = "ACTIVE"
        if session_data.get("customer"):
            tenant.stripe_customer_id = session_data.get("customer")
        if session_data.get("subscription"):
            tenant.stripe_subscription_id = session_data.get("subscription")
        db.update_tenant(tenant)

    updated = db.get_tenant(tenant.tenant_id)
    return {
        "status": "success",
        "message": f"Payment verified. Tenant successfully upgraded to {updated.subscription_plan}",
...
def scan_website_url(
    ...
    return {
        ...
        "site_context": result.get("site_context", {})
    }

@app.post("/api/v1/inspector/scan-pricing")
def scan_pricing_endpoint(
    req: ScanUrlReq,
    x_api_key: Optional[str] = Header(None, alias="X-API-Key")
):
    """Autonomously inspects a target website URL and returns detected pricing tiers and selectors."""
    from saas_platform.inspector.dom_scanner import DOMScanner
    plans = DOMScanner.scan_pricing_plans(req.url, html=req.html)
    return {
        "status": "success",
        "url": req.url,
        "plans": plans
...
def universal_assign_variants(
    ...
    if x_publishable_key:
        clean_pk = x_publishable_key.strip()
        tenant = db.get_tenant_by_publishable_key(clean_pk)
        if tenant and (tenant.tenant_id.startswith("tenant_design_partner_") or "design_partner" in clean_pk.lower()):
            check_partner_lock_status(tenant)
    if not tenant:
        tenant = db.get_tenant_by_api_key(Config.DEFAULT_API_KEY)
    if not tenant:
    ...
    # If previewing a specific variant, ensure the parent experiment is loaded even if paused
    if req.preview_variant:
        target_pv = req.preview_variant.lower().strip()
        for e_cand in experiments:
            if e_cand not in active_experiments:
                v_matches = db.list_variants_for_experiment(e_cand.experiment_id)
                for v in v_matches:
                    sanitized_name = v.name.lower().replace(" ", "_").replace(":", "").replace("-", "_")
                    if v.variant_id == req.preview_variant or v.opaque_id == req.preview_variant or target_pv in sanitized_name or target_pv == v.name.lower():
                        active_experiments.append(e_cand)
                        break
    # Strict Multi-Tenancy: Never leak another tenant's experiments if this tenant has none
    ...
    for exp in active_experiments:
        ...
        try:
            if req.preview_variant:
                ...
                # 1. Handle generic preview flags (?neri_preview=1, ?neri_preview=true, ?neri_preview=test)
                if target_key in ["1", "true", "yes", "preview", "test", "variant", "b", "v2"]:
                    non_controls = [v for v in variants if not v.is_control]
                    if non_controls:
                        match = non_controls[0]
                elif target_key in ["control", "baseline", "0"]:
                    controls = [v for v in variants if v.is_control]
                    if controls:
                        match = controls[0]

                # 2. Match exact variant ID, opaque ID, or title
                if not match:
                    for v in variants:
                        if v.variant_id == req.preview_variant or v.opaque_id == req.preview_variant:
                            match = v
                ...
                # 3. If preview requested but no specific variant matched, force first non-control test variant
                if not match and target_key != "control":
                    non_controls = [v for v in variants if not v.is_control]
                    if non_controls:
                        match = non_controls[0]
        ...
        assigned_selector = exp.target_selector
        if chosen_variant.pricing_payload and chosen_variant.pricing_payload.price_selector:
            assigned_selector = chosen_variant.pricing_payload.price_selector

        assignments.append({
            "experiment_id": exp.experiment_id,
            "tenant_id": tenant.tenant_id,
            "target_selector": assigned_selector,
...
def delete_experiment(
    ...
    # Cascade-delete behavioral rescue policies & founder discounts associated with this experiment
    db.delete_behavioral_rescue_config(tenant.tenant_id)

    return {
        "status": "success",
        "message": f"Experiment '{experiment_id}', its variants, and associated rescue policies have been deleted.",
...
def update_variant_content(
    ...
    if req.price_dollars is not None and variant.pricing_payload:
        variant.pricing_payload.price_amount_cents = int(req.price_dollars * 100)
    if req.plan_name is not None and variant.pricing_payload:
        variant.pricing_payload.plan_name = req.plan_name
...
# =====================================================================
# Design Partner Key Management Endpoints (Founder Superuser Only)
# =====================================================================

@app.get("/api/v1/admin/partners")
def list_design_partners(admin: Tenant = Depends(verify_founder_admin)):
    """Returns all 10 design partner accounts with live trial and lock status."""
    partners = []
    for i in range(1, 11):
        tid = f"tenant_design_partner_{i}"
        t = db.get_tenant(tid)
        if not t:
            db._seed_design_partner_tenants()
            t = db.get_tenant(tid)
        if t:
            ...
            if t.trial_expires_at:
                diff = t.trial_expires_at - time.time()
                days_left = max(0, round(diff / 86400, 1))
            is_locked = (
                t.subscription_status in ("LOCKED", "SUSPENDED", "TRIAL_EXPIRED") or
                (t.trial_expires_at is not None and time.time() > t.trial_expires_at)
            )
            partners.append({
...
@app.post("/api/v1/admin/partners/{partner_id}/lock")
def lock_design_partner(partner_id: str, admin: Tenant = Depends(verify_founder_admin)):
    """Immediately locks a design partner key, revoking dashboard, API, and SDK access."""
    t = db.get_tenant(partner_id)
    if not t:
        raise HTTPException(status_code=404, detail=f"Partner {partner_id} not found")
    t.subscription_status = "LOCKED"
    db.update_tenant(t)
    return {"status": "success", "message": f"{t.organization_name} has been locked.", "tenant_id": t.tenant_id, "subscription_status": "LOCKED"}


@app.post("/api/v1/admin/partners/{partner_id}/unlock")
def unlock_design_partner(partner_id: str, admin: Tenant = Depends(verify_founder_admin)):
    """Re-activates a design partner key."""
    t = db.get_tenant(partner_id)
    if not t:
        raise HTTPException(status_code=404, detail=f"Partner {partner_id} not found")
    t.subscription_status = "ACTIVE"
    if t.trial_expires_at and time.time() > t.trial_expires_at:
        t.trial_expires_at = time.time() + (14 * 86400)
    db.update_tenant(t)
    return {"status": "success", "message": f"{t.organization_name} has been unlocked.", "tenant_id": t.tenant_id, "subscription_status": "ACTIVE"}


@app.post("/api/v1/admin/partners/{partner_id}/set-trial")
def set_partner_trial(partner_id: str, req: SetTrialReq, admin: Tenant = Depends(verify_founder_admin)):
    """Sets or updates a trial duration (e.g. 14 or 30 days, or null for indefinite)."""
    t = db.get_tenant(partner_id)
    if not t:
        raise HTTPException(status_code=404, detail=f"Partner {partner_id} not found")
    if req.days is not None and req.days > 0:
        t.trial_expires_at = time.time() + (req.days * 86400)
        t.subscription_status = "ACTIVE"
    else:
        t.trial_expires_at = None
        t.subscription_status = "ACTIVE"
    db.update_tenant(t)
    return {"status": "success", "message": f"Trial updated for {t.organization_name}.", "tenant_id": t.tenant_id, "trial_expires_at": t.trial_expires_at}
