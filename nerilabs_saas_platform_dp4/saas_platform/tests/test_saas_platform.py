...
class TestDesignPartnerKeys(unittest.TestCase):
    ...
    def test_partner_locking_and_trial_expiration(self):
        """Verify that founder admin can lock/unlock partner keys and enforce trial expiration."""
        admin_headers = {"X-API-Key": "nerilabs_admin_master_2026_founder"}
        partner1_key = "nerilabs_design_partner_1"
        partner2_key = "nerilabs_design_partner_2"

        # 1. Partner cannot access founder partner management endpoint
        res_forbidden = self.client.get("/api/v1/admin/partners", headers={"X-API-Key": partner1_key})
        self.assertEqual(res_forbidden.status_code, 403)

        # 2. Founder can list all 10 partners
        res_list = self.client.get("/api/v1/admin/partners", headers=admin_headers)
        self.assertEqual(res_list.status_code, 200)
        partners = res_list.json()["partners"]
        self.assertEqual(len(partners), 10)

        # 3. Lock partner 1
        res_lock = self.client.post("/api/v1/admin/partners/tenant_design_partner_1/lock", headers=admin_headers)
        self.assertEqual(res_lock.status_code, 200)

        # 4. Partner 1 is rejected on login, API, and SDK calls
        res_login_locked = self.client.post("/api/v1/auth/admin-login", json={"admin_key": partner1_key})
        self.assertEqual(res_login_locked.status_code, 403)

        res_api_locked = self.client.get("/api/v1/tenant/me", headers={"X-API-Key": partner1_key})
        self.assertEqual(res_api_locked.status_code, 403)

        res_sdk_locked = self.client.post("/api/v1/experiments/universal-assign", json={
            "visitor_id": "vis_locked_test"
        }, headers={"X-Publishable-Key": partner1_key})
        self.assertEqual(res_sdk_locked.status_code, 403)

        # 5. Partner 2 is unaffected
        res_p2_login = self.client.post("/api/v1/auth/admin-login", json={"admin_key": partner2_key})
        self.assertEqual(res_p2_login.status_code, 200)

        # 6. Unlock partner 1
        res_unlock = self.client.post("/api/v1/admin/partners/tenant_design_partner_1/unlock", headers=admin_headers)
        self.assertEqual(res_unlock.status_code, 200)

        # Partner 1 works again
        res_login_unlocked = self.client.post("/api/v1/auth/admin-login", json={"admin_key": partner1_key})
        self.assertEqual(res_login_unlocked.status_code, 200)

        # 7. Test trial setting and expiration
        res_trial = self.client.post("/api/v1/admin/partners/tenant_design_partner_1/set-trial", json={"days": 14}, headers=admin_headers)
        self.assertEqual(res_trial.status_code, 200)

        # Simulate expired trial in database
        from saas_platform.models.database import db
        p1 = db.get_tenant("tenant_design_partner_1")
        p1.trial_expires_at = time.time() - 3600  # expired 1 hour ago
        db.update_tenant(p1)

        res_expired_login = self.client.post("/api/v1/auth/admin-login", json={"admin_key": partner1_key})
        self.assertEqual(res_expired_login.status_code, 403)
        self.assertIn("expired", res_expired_login.text.lower())

        # Reset partner 1 to active with no expiry for subsequent tests
        p1.trial_expires_at = None
        p1.subscription_status = "ACTIVE"
        db.update_tenant(p1)

    def test_strict_multi_tenant_isolation_between_founder_and_design_partners(self):
        """Verify that Founder Admin and each Design Partner ONLY ever see and manage their own experiments."""
        founder_headers = {"X-API-Key": "nerilabs_admin_master_2026_founder"}
        partner1_headers = {"X-API-Key": "nerilabs_design_partner_1"}
        partner2_headers = {"X-API-Key": "nerilabs_design_partner_2"}

        # 1. Founder creates an experiment
        res_f = self.client.post("/api/v1/experiments/create", json={
            "experiment_id": "exp_founder_isolated_001",
            "title": "Founder Only Hero Test",
            "experiment_type": "COPY_UI",
            "generation_mode": "FOUNDER_DIRECTED",
            "url": "https://nerilabs.io",
            "target_selector": "#hero-cta"
        }, headers=founder_headers)
        self.assertEqual(res_f.status_code, 200)

        # 2. Partner 1 creates an experiment
        res_p1 = self.client.post("/api/v1/experiments/create", json={
            "experiment_id": "exp_partner1_isolated_001",
            "title": "Partner 1 Pricing Test",
            "experiment_type": "COPY_UI",
            "generation_mode": "FOUNDER_DIRECTED",
            "url": "https://partner1.io",
            "target_selector": "#btn-buy"
        }, headers=partner1_headers)
        self.assertEqual(res_p1.status_code, 200)

        # 3. Partner 2 creates an experiment
        res_p2 = self.client.post("/api/v1/experiments/create", json={
            "experiment_id": "exp_partner2_isolated_001",
            "title": "Partner 2 Feature Test",
            "experiment_type": "COPY_UI",
            "generation_mode": "FOUNDER_DIRECTED",
            "url": "https://partner2.io",
            "target_selector": "#signup"
        }, headers=partner2_headers)
        self.assertEqual(res_p2.status_code, 200)

        # 4. Verify Partner 1 ONLY sees Partner 1's experiment
        res_p1_list = self.client.get("/api/v1/experiments/tenant/list", headers=partner1_headers)
        self.assertEqual(res_p1_list.status_code, 200)
        p1_exp_ids = [e["experiment_id"] for e in res_p1_list.json()]
        self.assertIn("exp_partner1_isolated_001", p1_exp_ids)
        self.assertNotIn("exp_founder_isolated_001", p1_exp_ids)
        self.assertNotIn("exp_partner2_isolated_001", p1_exp_ids)

        # 5. Verify Partner 2 ONLY sees Partner 2's experiment
        res_p2_list = self.client.get("/api/v1/experiments/tenant/list", headers=partner2_headers)
        self.assertEqual(res_p2_list.status_code, 200)
        p2_exp_ids = [e["experiment_id"] for e in res_p2_list.json()]
        self.assertIn("exp_partner2_isolated_001", p2_exp_ids)
        self.assertNotIn("exp_founder_isolated_001", p2_exp_ids)
        self.assertNotIn("exp_partner1_isolated_001", p2_exp_ids)

        # 6. Verify Founder Admin ONLY sees Founder Admin's experiment
        res_f_list = self.client.get("/api/v1/experiments/tenant/list", headers=founder_headers)
        self.assertEqual(res_f_list.status_code, 200)
        f_exp_ids = [e["experiment_id"] for e in res_f_list.json()]
        self.assertIn("exp_founder_isolated_001", f_exp_ids)
        self.assertNotIn("exp_partner1_isolated_001", f_exp_ids)
        self.assertNotIn("exp_partner2_isolated_001", f_exp_ids)

        # 7. Verify Partner 1 CANNOT delete or mutate Founder's or Partner 2's experiment
        res_del_cross = self.client.delete("/api/v1/experiments/exp_founder_isolated_001", headers=partner1_headers)
        self.assertEqual(res_del_cross.status_code, 403)

        res_del_cross2 = self.client.delete("/api/v1/experiments/exp_partner2_isolated_001", headers=partner1_headers)
        self.assertEqual(res_del_cross2.status_code, 403)

    def test_upgrade_requires_stripe_configuration_and_verification(self):
        """Verify that plan upgrades are not granted for free without Stripe verification."""
        from saas_platform.models.database import db
        test_t = db.create_tenant(organization_name="Upgrade Test Org", api_key="nerilabs_sk_upgrade_test_123", subscription_plan="LAUNCH")
        test_headers = {"X-API-Key": test_t.api_key}

        # Attempt to create checkout when Stripe is unconfigured
        res_upg = self.client.post("/api/v1/billing/create-upgrade-checkout", json={"plan": "growth"}, headers=test_headers)
        # Should return 400 with configuration notice, NEVER a free bypass
        self.assertEqual(res_upg.status_code, 400)
        self.assertIn("Stripe", res_upg.json()["detail"])

        # Attempt to confirm upgrade with invalid / missing session
        res_conf = self.client.post("/api/v1/billing/confirm-upgrade", json={"plan": "scale", "session_id": ""}, headers=test_headers)
        self.assertEqual(res_conf.status_code, 400)

        # Verify tenant plan was NOT upgraded
        res_me = self.client.get("/api/v1/tenant/me", headers=test_headers)
        self.assertEqual(res_me.json()["subscription_plan"], "LAUNCH")
