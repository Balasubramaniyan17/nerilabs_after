/**
 * NeriLabs Universal Autonomous Experimentation Client SDK (v3.2 Production)
 ...
 * - Multi-action Conversion Tracking: Direct CTA clicks + Global Pricing/Checkout tracking
...
(function() {
    ...
    function flushBeacon() {
        ...
        // Use keepalive fetch first, with sendBeacon fallback for guaranteed delivery
        fetch(endpoint, {
            method: "POST",
            headers: {
        ...
        .then(res => {
            if (!res.ok) console.debug("[NeriLabs Telemetry] Beacon response:", res.status);
        })
        .catch(() => {
    ...
    // Helper: Trigger conversion on an assignment
    function recordConversion(assignment, clickedEl) {
        queueEvent(
            ...
            "CONVERSION",
            {
                ...
                clicked_text: clickedEl ? (clickedEl.innerText || clickedEl.textContent || "").substring(0, 40).trim() : "CTA Click",
                selector: assignment.target_selector
            },
            assignment.tenant_id
        );
        flushBeacon();
        console.info(`[NeriLabs Telemetry] Conversion registered for "${assignment.experiment_id}" on "${assignment.variant_name}"`);
    }
    ...
    function applySingleAssignment(assignment, el) {
        ...
        // 1. Direct Conversion tracking on this target element
        el.addEventListener("click", function() {
            recordConversion(assignment, el);
        }, { capture: true });
    }

    // Observe DOM until target element mounts (supporting React, Vue, Webflow, dynamic SPAs)
    function observeAndApply(assignment, maxWaitMs = 4000) {
        let applied = false;
        function tryApply() {
            if (applied) return true;
            const elements = resolveElements(assignment.target_selector);
    ...
    // 2. Global Conversion Delegation:
    // Captures pricing clicks, plan selections, and checkout CTAs across the entire page!
    document.addEventListener("click", function(ev) {
        ...
        const isConversionAction = target.hasAttribute("data-plan") || 
                                   target.classList.contains("plan-cta") ||
                                   ...
                                   text.includes("$") || 
                                   text.includes("/mo") ||
                                   target.closest(".price-card") !== null;

        if (isConversionAction && activeAssignments.length > 0) {
