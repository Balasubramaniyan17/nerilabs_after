...
class MABEngine:
    ...
    def _load_arms(self):
        ...
        for v in variants:
            if (v.status in ["APPROVED", "ACTIVE"] or v.is_control) and v.status != "REJECTED":
    ...
    def register_variant(self, variant: Variant):
        if not variant.is_control and variant.status not in ["APPROVED", "ACTIVE"]:
            return  # Do not register unapproved variants into active traffic
    ...
    def select_variant_for_visitor(self, visitor_id: str) -> Tuple[Variant, str, float]:
        ...
        active_arms = [
            ...
            if (a["variant"].is_control or a["variant"].status in ["APPROVED", "ACTIVE"]) and a["variant"].status != "REJECTED"
        ]

        if not active_arms:
            exp = db.get_experiment(self.experiment_id)
            if exp:
                ...
                raw_vars = AgentOrchestrator.generate_and_cache_experiment_suite(exp)
                for v in raw_vars:
                    rep = GuardrailEngine.validate_variant(v, exp)
                    if rep.is_safe:
                        if v.is_control:
        ...
        if not active_arms:
            safe_ctrl = Variant(
                variant_id=f"var_ctrl_{self.experiment_id[:8]}",
                experiment_id=self.experiment_id,
                ...
                copy_payload={"new_text": "Get Started"}
            )
            return safe_ctrl, "fallback_control", 0.5

        now = time.time()
        for arm in active_arms:
            self._apply_time_decay(arm, now)

        # 1. Warm-up phase: Ensure all arms (including Control) get baseline traffic evenly distributed
        under_explored = [a for a in active_arms if a["impressions"] < 10]
        if under_explored:
            # Deterministic consistent hashing on visitor_id across the least-explored arms
            # This guarantees 50/50 split between Control and Variant(s) for different visitors
            min_impr = min(a["impressions"] for a in active_arms)
            candidate_pool = [a for a in active_arms if a["impressions"] <= min_impr + 2]
            
            import hashlib
            hash_val = int(hashlib.md5(f"{self.experiment_id}:{visitor_id}".encode()).hexdigest(), 16)
            chosen = candidate_pool[hash_val % len(candidate_pool)]
            return chosen["variant"], "balanced_warmup", 0.5
    ...
    def sync_from_database(self):
        ...
        try:
            ...
            for v_id, arm in self.arms.items():
                ...
                if cur_impr > arm["impressions"] or cur_conv > arm["conversions"]:
                    arm["impressions"] = max(arm["impressions"], cur_impr)
                    arm["conversions"] = max(arm["conversions"], cur_conv)
                    arm["alpha"] = 1.0 + float(arm["conversions"])
                    arm["beta_param"] = max(1.0, 1.0 + float(arm["impressions"] - arm["conversions"]))
                    self._persist_arm(arm)
        except Exception as e:
            print("ERROR IN ANALYTICS CALC:", e)

    def unregister_variant(self, variant_id: str):
        if variant_id in self.arms:
            del self.arms[variant_id]
        conn = db._get_connection()
        with conn:
            conn.execute("DELETE FROM bandit_arms WHERE experiment_id = ? AND variant_id = ?", (self.experiment_id, variant_id))
    ...
    def compute_analytics(self) -> MABAnalyticsReport:
        ...
        # Compute bot filtering & traffic source metrics from telemetry_events
        bot_traffic_filtered = 0
        human_traffic_count = total_impressions
        traffic_by_source = {}

        try:
            conn = db._get_connection()
            rows = conn.execute("SELECT event_type, metadata_json FROM telemetry_events WHERE experiment_id = ?", (self.experiment_id,)).fetchall()
            for r in rows:
                ...
                if r["metadata_json"]:
                    try:
                        ...
                        meta = {}
                if meta.get("is_bot", False):
                    if ev_type == "IMPRESSION":
                        bot_traffic_filtered += 1
                else:
                    src = meta.get("utm_source") or meta.get("source") or "Direct"
                    src_norm = "Google" if "google" in src.lower() else ("Instagram" if "instagram" in src.lower() else ("Facebook" if ("facebook" in src.lower() or "fb" in src.lower()) else ("LinkedIn" if "linkedin" in src.lower() else ("Twitter / X" if ("twitter" in src.lower() or "t.co" in src.lower() or "x.com" in src.lower()) else src.capitalize()))))
                    if src_norm not in traffic_by_source:
                        traffic_by_source[src_norm] = {
                            "impressions": 0,
                            "conversions": 0,
                            "cvr": 0.0,
                            "category": "CASUAL_SOCIAL" if src_norm in ["Instagram", "Facebook", "Tiktok"] else "HIGH_INTENT"
                        }
                    if ev_type == "IMPRESSION":
                        traffic_by_source[src_norm]["impressions"] += 1
                    elif ev_type == "CONVERSION":
                        traffic_by_source[src_norm]["conversions"] += 1

            for s_data in traffic_by_source.values():
                s_data["cvr"] = round(s_data["conversions"] / max(1, s_data["impressions"]), 4)
        except Exception as e:
            print("ERROR IN ANALYTICS CALC:", e)
        ...
        return MABAnalyticsReport(
            ...
            bot_traffic_filtered=bot_traffic_filtered,
            human_traffic_count=human_traffic_count,
