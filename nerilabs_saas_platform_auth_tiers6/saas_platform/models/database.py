...
class Database:
    ...
    def _migrate_schema_if_needed(self):
        ...
        with conn:
            ...
            if "auth0_sub" not in cols:
                try:
                    conn.execute("ALTER TABLE tenants ADD COLUMN auth0_sub TEXT;")
                except Exception: pass
            conn.execute("CREATE INDEX IF NOT EXISTS idx_tenant_sub ON tenants(auth0_sub);")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_tenant_email ON tenants(contact_email);")
    ...
    def purge_demo_experiment(self):
        ...
        with conn:
            ...
            conn.execute("DELETE FROM bandit_arms WHERE experiment_id = 'exp_saas_hero_conversion'")
    ...
    def clean_all_simulated_metrics(self):
        """Wipes all simulated telemetry events and locks arm states to authentic real-time events only."""
        conn = self._get_connection()
        with conn:
            # Delete demo experiment completely
            conn.execute("DELETE FROM telemetry_events WHERE experiment_id = 'exp_saas_hero_conversion'")
            conn.execute("DELETE FROM bandit_arms WHERE experiment_id = 'exp_saas_hero_conversion'")
            conn.execute("DELETE FROM variants WHERE experiment_id = 'exp_saas_hero_conversion'")
            conn.execute("DELETE FROM experiments WHERE experiment_id = 'exp_saas_hero_conversion'")

            # Delete any synthetic / simulated visitor sessions
            conn.execute("DELETE FROM telemetry_events WHERE visitor_id LIKE 'sim_%' OR visitor_id LIKE 'seed_%' OR visitor_id LIKE 'vis_sim_%'")

            # Remove duplicate variant rows for the same experiment and name
            conn.execute("""
            DELETE FROM variants WHERE rowid NOT IN (
                SELECT MAX(rowid) FROM variants GROUP BY experiment_id, name
            )
            """)

            # Reset arm impressions and conversions to match actual real telemetry events!
            arms = conn.execute("SELECT experiment_id, variant_id FROM bandit_arms").fetchall()
            for arm in arms:
                exp_id = arm["experiment_id"]
                var_id = arm["variant_id"]
                row_var = conn.execute("SELECT opaque_id FROM variants WHERE variant_id = ?", (var_id,)).fetchone()
                opaque_id = row_var["opaque_id"] if row_var else var_id

                cur_impr = conn.execute("""
                    SELECT COUNT(*) FROM telemetry_events 
                    WHERE experiment_id = ? AND (opaque_variant_id = ? OR opaque_variant_id = ?) AND event_type = 'IMPRESSION'
                """, (exp_id, var_id, opaque_id)).fetchone()[0]

                cur_conv = conn.execute("""
                    SELECT COUNT(*) FROM telemetry_events 
                    WHERE experiment_id = ? AND (opaque_variant_id = ? OR opaque_variant_id = ?) AND event_type = 'CONVERSION'
                """, (exp_id, var_id, opaque_id)).fetchone()[0]

                conn.execute("""
                    UPDATE bandit_arms 
                    SET impressions = ?, conversions = ?, alpha = 1.0 + ?, beta_param = 1.0 + (? - ?)
                    WHERE experiment_id = ? AND variant_id = ?
                """, (cur_impr, cur_conv, float(cur_conv), float(cur_impr), float(cur_conv), exp_id, var_id))

    def _seed_live_experiment(self, tenant_id: str):
        import json, uuid
        conn = self._get_connection()
        with conn:
            cur = conn.execute("SELECT experiment_id FROM experiments WHERE tenant_id = ?", (tenant_id,))
            if not cur.fetchone():
                exp_id = f"exp_{tenant_id}_conversion"
                conn.execute("""
                ...
                """, (
                    exp_id,
                    tenant_id,
                    "NeriLabs Live Website Optimization",
                    "COPY_UI",
                    "FOUNDER_DIRECTED",
                    "https://mellow-parfait-e103d4.netlify.app",
                    "a[href='#pricing'], .hero-ctas .btn-primary, #navActionBtn",
                    json.dumps({
                        "tag": "a",
                        "element_id": "navActionBtn",
                        "selector": "a[href='#pricing'], .hero-ctas .btn-primary, #navActionBtn",
                        "inner_text": "Start your first test"
                    }),
                    json.dumps({
                        "brand_name": "NeriLabs",
                ...
                sample_variants = [
                    ("var_ctrl_" + tenant_id[:6], "Control (Baseline)", "COPY", True, "Baseline unchanged user experience.", {"selector": "a[href='#pricing'], .hero-ctas .btn-primary, #navActionBtn", "original_text": "Start your first test", "new_text": "Start your first test", "trigger": "DEFAULT"}, None, 18, 1),
                    ("var_soc_" + tenant_id[:6], "Copy: Social Proof", "COPY", False, "Social proof framing increases trust.", {"selector": "a[href='#pricing'], .hero-ctas .btn-primary, #navActionBtn", "original_text": "Start your first test", "new_text": "Join 2,500+ Founders · Start Free", "trigger": "SOCIAL_PROOF"}, None, 14, 2),
                    ("var_urg_" + tenant_id[:6], "Copy: Urgency", "COPY", False, "Urgency motivates immediate setup.", {"selector": "a[href='#pricing'], .hero-ctas .btn-primary, #navActionBtn", "original_text": "Start your first test", "new_text": "Start Your Test Today — Deploy in 2 Min", "trigger": "URGENCY"}, None, 12, 1),
                    ("var_fric_" + tenant_id[:6], "Copy: Friction Reduction", "COPY", False, "Emphasizing no credit card reduces resistance.", {"selector": "a[href='#pricing'], .hero-ctas .btn-primary, #navActionBtn", "original_text": "Start your first test", "new_text": "Start Free (No Credit Card Required)", "trigger": "FRICTION_REDUCTION"}, None, 15, 2),
                    ("var_clar_" + tenant_id[:6], "Copy: Clarity", "COPY", False, "Clear outcome framing clarifies value.", {"selector": "a[href='#pricing'], .hero-ctas .btn-primary, #navActionBtn", "original_text": "Start your first test", "new_text": "Try NeriLabs Free — Deploy First Test", "trigger": "CLARITY"}, None, 10, 1),
                ]
                for vid, name, vtype, is_ctrl, hyp, c_payload, s_payload, impr, conv in sample_variants:
                    opq = uuid.uuid4().hex[:12]
                    conn.execute("""
                    ...
                    """, (
                        vid, opq, exp_id, tenant_id, name, vtype,
                        "FOUNDER_DIRECTED", hyp,
                        json.dumps(c_payload) if c_payload else None,
                        json.dumps(s_payload) if s_payload else None,
                        None, None, None,
                        1 if is_ctrl else 0, "APPROVED", time.time()
                    ))
                    conn.execute("""
                    ...
                    """, (
                        exp_id, vid, 1.0 + float(conv), 1.0 + float(impr - conv), impr, conv, time.time()
                    ))

    def _seed_admin_tenant(self):
        conn = self._get_connection()
        with conn:
            cur = conn.execute("SELECT tenant_id FROM tenants WHERE tenant_id = 'tenant_founder_admin'")
            if not cur.fetchone():
                conn.execute("""
                ...
                """, (
                    "tenant_founder_admin",
                    "NeriLabs Founder (Master Admin)",
                    "founder@nerilabs.io",
                    Config.ADMIN_MASTER_KEY,
                    "nerilabs_pk_admin_master",
                    "admin-token-signing-secret-key-2026",
                    "ENTERPRISE",
                    "ACTIVE",
                    1,
                    1,
        ...
        self._seed_live_experiment("tenant_founder_admin")
    ...
    def _seed_default_tenant(self):
        ...
        self._seed_live_experiment("tenant_startup_01")

    # =========================================================================
    # Tenant Operations
    # =========================================================================

    def get_tenant_by_auth0_sub(self, sub: str) -> Optional[Tenant]:
        if not sub: return None
        conn = self._get_connection()
        row = conn.execute("SELECT * FROM tenants WHERE auth0_sub = ?", (sub,)).fetchone()
        return self._row_to_tenant(row) if row else None

    def get_tenant_by_email(self, email: str) -> Optional[Tenant]:
        if not email: return None
        conn = self._get_connection()
        row = conn.execute("SELECT * FROM tenants WHERE contact_email = ?", (email,)).fetchone()
        return self._row_to_tenant(row) if row else None
    ...
    def create_tenant(
        ...
        with conn:
            conn.execute("""
            INSERT INTO tenants (
                tenant_id, organization_name, contact_email, auth0_sub, api_key, publishable_key,
            ...
            """, (
                tenant_id, organization_name, contact_email, auth0_sub, secret_api_key, pub_key,
    ...
    def _row_to_tenant(self, row: sqlite3.Row) -> Tenant:
        from saas_platform.models.schemas import get_plan_limits
        col_names = row.keys()
        sub_plan = row["subscription_plan"] or "GROWTH"
        return Tenant(
            tenant_id=row["tenant_id"],
            organization_name=row["organization_name"],
            contact_email=row["contact_email"],
            auth0_sub=row["auth0_sub"] if "auth0_sub" in col_names else None,
            api_key=row["api_key"],
            ...
            plan_limits=get_plan_limits(sub_plan)
        )
