    ...
    try {
        ...
        if (resOverall.ok) {
            ...
            document.getElementById("stat-leading-name").textContent = overall.leading_variant_name ? `Leader: ${overall.leading_variant_name}` : "Exploring...";
                ...
                // Update MAB Status Banner based on actual impressions
        ...
        if (totalCount === 0 || (overall && overall.total_impressions === 0)) {
            if (trustBanner) {
                trustBanner.className = "alert-box alert-warning";
                if (statusTitle) statusTitle.textContent = "📡 Listening for Live Website Visitors";
                if (statusDesc) statusDesc.textContent = "Script is active. Visit your website or click your CTA button to start tracking live impressions and conversions in real time.";
            }
        } else {
            if (trustBanner) {
                trustBanner.className = "alert-box alert-warning";
                if (statusTitle) statusTitle.textContent = "⚡ Thompson Sampling Active";
                if (statusDesc) statusDesc.textContent = "Algorithm is dynamically allocating traffic to maximize conversions with a 10% exploration floor.";
            }
        }
