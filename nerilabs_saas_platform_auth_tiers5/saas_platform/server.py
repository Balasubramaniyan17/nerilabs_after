...
def handle_onboarding_setup(
    ...
    limits = get_plan_limits(tenant.subscription_plan)
    is_admin = (tenant.tenant_id == "tenant_founder_admin" or tenant.api_key == Config.ADMIN_MASTER_KEY or tenant.subscription_plan == "ENTERPRISE")
    if not is_admin:
...
def synthesize_combinations(
    ...
    is_admin = (tenant.tenant_id == "tenant_founder_admin" or tenant.api_key == Config.ADMIN_MASTER_KEY or tenant.subscription_plan == "ENTERPRISE")
    limits = get_plan_limits(tenant.subscription_plan)
    if not is_admin and not limits["has_factorial"]:
        raise HTTPException(
            status_code=403,
