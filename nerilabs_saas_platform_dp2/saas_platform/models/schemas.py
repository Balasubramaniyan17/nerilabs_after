...
class SubscriptionPlan(str, Enum):
    LAUNCH = "LAUNCH"          # $0/mo Free (1 live experiment, 5k visits)
    STARTER = "STARTER"        # $49/mo (5 live experiments, 50k visits)
    GROWTH = "GROWTH"          # $99/mo (20 live experiments, 250k visits, Stripe pricing)
    SCALE = "SCALE"            # $199/mo (unlimited experiments, 1M visits, anomalies + factorial)
    ENTERPRISE = "ENTERPRISE"  # Unlimited / Founder Master Admin
...
PLAN_LIMITS: Dict[str, Dict[str, Any]] = {
    "LAUNCH": {
        ...
        "max_experiments": 1,
        "max_traffic": 5000,
        "allowed_experiment_types": ["COPY_UI"],
        "has_stripe_pricing": False,
        "has_onboarding_branching": False,
        "has_anomalies": False,
        "has_factorial": False,
        "has_audit_override": False,
        "is_admin": False
    },
    "STARTER": {
        "name": "Starter Plan",
    ...
    "GROWTH": {
        ...
        "max_experiments": 20,
        "max_traffic": 250000,
        "allowed_experiment_types": ["COPY_UI", "PRICING_TEST", "ONBOARDING_FUNNEL"],
        "has_stripe_pricing": True,
        "has_onboarding_branching": True,
        "has_anomalies": False,
        "has_factorial": False,
        "has_audit_override": False,
        "is_admin": False
...
def get_plan_limits(plan_name: Optional[str]) -> Dict[str, Any]:
    norm = (plan_name or "LAUNCH").upper().strip()
    if "ENTERPRISE" in norm or "ADMIN" in norm:
...
class MABAnalyticsReport(BaseModel):
    ...
    bot_traffic_filtered: int = 0
    human_traffic_count: int = 0
    traffic_by_source: Dict[str, Any] = Field(default_factory=dict)
...
class StripeSettingsReq(BaseModel):
...
class BehavioralRescueConfig(BaseModel):
    ...
    dwell_threshold_seconds: float = 6.0
...
class UniversalAssignReq(BaseModel):
    visitor_id: str
    session_id: Optional[str] = None
    device_type: Optional[str] = "desktop"
    url: Optional[str] = None
...
class Auth0SyncReq(BaseModel):
    sub: str
    email: str
    name: Optional[str] = None
    plan: Optional[str] = "LAUNCH"


class AdminLoginReq(BaseModel):
    admin_key: str

class SetTrialReq(BaseModel):
