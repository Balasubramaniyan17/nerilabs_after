...
async function fetchAccountInfo() {
    ...
    try {
        ...
        if (partnerNav) {
            const isFounder = IS_MASTER_ADMIN && (!CURRENT_TENANT || CURRENT_TENANT.tenant_id === "tenant_founder_admin" || CURRENT_TENANT.api_key.includes("admin"));
            partnerNav.style.display = isFounder ? "flex" : "none";
        }
                    ...
                    vList.innerHTML = vars.map(v => {
                        ...
                        return `
                            <div style="display: flex; justify-content: space-between; align-items: center; background: var(--panel-1); border: 1px solid var(--line); border-radius: 3px; padding: 6px 10px; gap: 8px;">
                                ...
                                <button type="button" class="btn btn-secondary btn-sm" onclick="openEditVariantModal('${v.variant_id}')" style="font-size: 10.5px; padding: 3px 8px; cursor: pointer;">✏️ Edit</button>
                            </div>
                        `;
                    }).join("");
                }
    ...
    try {
        ...
        tbody.innerHTML = variants.map(v => {
            ...
            if (isControl) {
                ...
                const editBtn = `<button class="btn btn-sm btn-secondary" onclick="openEditVariantModal('${v.variant_id}')" style="font-size: 11px; padding: 4px 10px; cursor: pointer;" title="Edit proposed copy and hypothesis">✏️ Edit Copy</button>`;
...
async function loadDesignPartnersList() {
    ...
    try {
        ...
        if (!res.ok) {
            tbody.innerHTML = '<tr><td colspan="5" style="padding:20px; text-align:center; color:var(--alert);">Failed to load partners: ' + res.statusText + '</td></tr>';
            return;
        }
        const data = await res.json();
        ...
        for (const p of partners) {
            ...
            if (p.trial_expires_at) {
                trialDisplay = p.days_left > 0 ? (p.days_left + ' days left') : '<span style="color:var(--alert);">Trial Expired</span>';
            }
            ...
            html += '<tr style="border-bottom: 1px solid var(--border); font-size: 12px;">' +
                '<td style="padding: 12px 10px;"><strong>' + p.organization_name + '</strong><br><small style="color:var(--ink-dim); font-family:monospace;">' + p.tenant_id + '</small></td>' +
                ...
                '<td style="padding: 12px 10px; text-align: right;">' +
                    '<div style="display:flex; gap:6px; justify-content:flex-end;">' +
                        actionBtn +
                        '<button class="btn btn-secondary" style="padding:4px 8px; font-size:11px;" onclick="promptSetTrial('' + p.tenant_id + '', '' + p.organization_name + '')">Set Trial</button>' +
                    '</div>' +
                '</td>' +
            '</tr>';
        }
        ...
        tbody.innerHTML = '<tr><td colspan="5" style="padding:20px; text-align:center; color:var(--alert);">' + err.message + '</td></tr>';
    }
}
...
async function promptSetTrial(tenantId, orgName) {
    const input = prompt("Set trial duration in days for " + orgName + " (e.g. 14, 30, or leave blank for unlimited):");
