...
class FrictionEngine:
    ...
    def evaluate_behavioral_event(
        ...
        # 2. Determine target tier, baseline price, and target selector from config
        target_tier = (getattr(config, "target_tier", "AUTO") or "AUTO").upper()
        target_selector = ""
        tier_name = "Growth Plan"
        baseline_price_cents = 9900

        if target_tier == "STARTER":
            baseline_price_cents = 4900
            tier_name = "Starter Plan"
            target_selector = ".pricing-grid > :nth-child(1) .plan-price, .pricing-grid > :nth-child(1) .price-amount, .price-card:nth-child(1) .price-amount"
        elif target_tier == "GROWTH":
            baseline_price_cents = 9900
            tier_name = "Growth Plan"
            target_selector = ".pricing-grid > :nth-child(2) .plan-price, .pricing-grid > :nth-child(2) .price-amount, .price-card.mid .price-amount"
        elif target_tier == "SCALE":
            baseline_price_cents = 19900
            tier_name = "Scale Plan"
            target_selector = ".pricing-grid > :nth-child(3) .plan-price, .pricing-grid > :nth-child(3) .price-amount, .price-card:nth-child(3) .price-amount"
        else:
            # AUTO: Resolve tier from active experiment
            if experiment and experiment.target_selector:
                target_selector = experiment.target_selector
                exp_vars = db.list_variants_for_experiment(experiment.experiment_id)
                for v in exp_vars:
                    if v.is_control and v.pricing_payload:
                        baseline_price_cents = v.pricing_payload.price_amount_cents
                        tier_name = v.pricing_payload.plan_name
                        if v.pricing_payload.price_selector:
                            target_selector = v.pricing_payload.price_selector
                        break
            if not target_selector:
                target_selector = ".pricing-grid > :nth-child(2) .plan-price, .pricing-grid > :nth-child(2) .price-amount, .price-card.mid .price-amount"

        # 3. Invoke BehavioralOfferAgent to synthesize creative personalized offers
        ...
        discount_pct = selected_offer.get("discount_percent", config.discount_percent or 20)
        promo_code = selected_offer.get("promo_code", config.promo_code or f"PROMO-{secrets.token_hex(2).upper()}")
        ...
        # Resolve Stripe credentials (configured or deterministic)
        stripe_price_id = getattr(config, "stripe_price_id", None) or f"price_stripe_{event.tenant_id[:6]}_{discounted_price_cents}_{promo_code.replace('-', '_').lower()}"
        stripe_payment_link = getattr(config, "stripe_payment_link", None)

        # 4. Create concession variant with RESCUE_ONLY status (NEVER served by MAB to cold visitors and NEVER attached to experiment)
        rescue_exp_id = f"exp_rescue_{event.tenant_id}"
        personalized_variant = Variant(
            variant_id=f"var_rescue_concession_{discounted_price_cents}_{secrets.token_hex(3)}",
            opaque_id=f"opq_rescue_{secrets.token_hex(4)}",
            experiment_id=rescue_exp_id,
            tenant_id=event.tenant_id,
            name=f"Rescue Concession: {promo_code} ({discount_pct}% OFF)",
            variant_type=VariantType.PRICING,
            hypothesis=f"Personalized concession for {tier_name} converts hesitating visitor.",
            pricing_payload=PricingPlan(
                plan_name=f"{tier_name} ({promo_code})",
                price_amount_cents=discounted_price_cents,
                currency="usd",
                interval="month",
                stripe_price_id=stripe_price_id,
                stripe_payment_link=stripe_payment_link,
                price_selector=target_selector
            ),
            status="RESCUE_ONLY"
        )
        db.save_variant(personalized_variant)

        # 4. In-Place Token Escalation (Fallback to fresh token generation if untokenized)
        ...
        if not success or not new_signed_token:
            new_signed_token, updated_payload = TokenService.issue_new_token(
                tenant_id=experiment.tenant_id,
                experiment_id=experiment.experiment_id,
                session_id=sess_id,
                visitor_id=event.visitor_id,
                variant=personalized_variant
            )
            success = True
        ...
        if success:
            ...
            payload_data = {
                ...
                "original_price_formatted": f"${baseline_price_cents/100:.0f}/mo",
