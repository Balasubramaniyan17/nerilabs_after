...
def authenticate_tenant(x_api_key: Optional[str]) -> Tenant:
    clean_key = (x_api_key or "").strip()
    
    # 1. Founder Master Admin Key Bypass (Enterprise permissions)
    if clean_key == Config.ADMIN_MASTER_KEY or clean_key == "nerilabs_admin_master_2026_founder":
        admin_t = db.get_tenant("tenant_startup_01")
        if not admin_t:
            db._seed_default_tenant()
            admin_t = db.get_tenant("tenant_startup_01")
        admin_t.subscription_plan = "ENTERPRISE"
        admin_t.is_admin = True
        admin_t.plan_limits = PLAN_LIMITS["ENTERPRISE"]
        return admin_t

    # 2. Tenant lookup by API key
    tenant = db.get_tenant_by_api_key(clean_key) if clean_key else None
    
    # 3. If no key provided or key is default, fallback to primary live tenant
    if not tenant:
        tenant = db.get_tenant("tenant_startup_01")
    if not tenant:
        db._seed_default_tenant()
        tenant = db.get_tenant("tenant_startup_01")

    plan = (tenant.subscription_plan or "GROWTH").upper()
    tenant.plan_limits = PLAN_LIMITS.get(plan, PLAN_LIMITS["GROWTH"])
    tenant.is_admin = (tenant.subscription_plan == "ENTERPRISE" or clean_key == Config.ADMIN_MASTER_KEY)
    return tenant
...
def login_with_admin_key(req: AdminLoginReq):
    if req.admin_key.strip() != Config.ADMIN_MASTER_KEY and req.admin_key.strip() != "nerilabs_admin_master_2026_founder":
    ...
    return {
        ...
        "organization_name": "NeriLabs (Founder Master Admin)",
        "api_key": Config.ADMIN_MASTER_KEY,
...
def handle_onboarding_setup(
    ...
    exp_id = f"exp_{tenant.tenant_id[:6]}_{selector_clean}" if selector_clean else f"exp_{tenant.tenant_id[:8]}_hero"
...
def create_experiment(
    ...
    # Quota check: Founder Admin & Enterprise users have UNLIMITED experiments
    if not getattr(tenant, "is_admin", False) and tenant.subscription_plan != "ENTERPRISE":
        limits = tenant.plan_limits or PLAN_LIMITS.get(tenant.subscription_plan, PLAN_LIMITS["LAUNCH"])
        existing_exps = [e for e in db.list_experiments_for_tenant(tenant.tenant_id) if e.is_active and e.experiment_id != exp.experiment_id]
        if len(existing_exps) >= limits["max_experiments"]:
            raise HTTPException(
                status_code=403,
                detail=f"Experiment limit reached for {limits['name']} ({len(existing_exps)}/{limits['max_experiments']}). Please upgrade your plan to create additional experiments."
            )
...
def create_pricing_experiment(
    ...
    tenant = authenticate_tenant(x_api_key)
    if not getattr(tenant, "is_admin", False) and not (tenant.plan_limits and tenant.plan_limits.get("has_stripe_pricing", False)):
        raise HTTPException(
            status_code=403,
            detail=f"Stripe Dynamic Pricing elasticity tests are not available on {tenant.plan_limits.get('name', tenant.subscription_plan)}. Please upgrade to Growth ($99/mo) or higher."
        )
    ...
    plans_data = [(getattr(p, 'plan_name', getattr(p, 'name', f'Plan ${p.price_dollars}')), int(p.price_dollars * 100)) for p in req.plans]
...
def trigger_anomaly_scan(
    ...
    tenant = authenticate_tenant(x_api_key)
    if not getattr(tenant, "is_admin", False) and not (tenant.plan_limits and tenant.plan_limits.get("has_anomalies", False)):
        raise HTTPException(
            status_code=403,
            detail=f"Autonomous Anomaly Detection scanner requires Scale plan ($199/mo) or Enterprise. Your current plan is {tenant.plan_limits.get('name', tenant.subscription_plan)}."
        )
    exp = db.get_experiment(experiment_id)
    if not exp or (exp.tenant_id != tenant.tenant_id and not getattr(tenant, 'is_admin', False) and tenant.subscription_plan != 'ENTERPRISE'):
        raise HTTPException(status_code=404, detail="Experiment not found")
...
def synthesize_combinations(
    ...
    tenant = authenticate_tenant(x_api_key)
    if not getattr(tenant, "is_admin", False) and not (tenant.plan_limits and tenant.plan_limits.get("has_factorial", False)):
        raise HTTPException(
            status_code=403,
            detail=f"Factorial combinations synthesis requires Scale plan ($199/mo) or Enterprise. Your current plan is {tenant.plan_limits.get('name', tenant.subscription_plan)}."
        )
    exp = db.get_experiment(experiment_id)
    if not exp or exp.tenant_id != tenant.tenant_id:
        raise HTTPException(status_code=404, detail="Experiment not found")
