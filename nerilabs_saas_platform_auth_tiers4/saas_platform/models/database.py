...
class Database:
    ...
    def get_tenant_by_email(self, email: str) -> Optional[Tenant]:
        ...
        conn = self._get_connection()
        row = conn.execute("SELECT * FROM tenants WHERE LOWER(contact_email) = LOWER(?)", (email.strip(),)).fetchone()
        return self._row_to_tenant(row) if row else None
    ...
    def create_tenant(
        ...
        key = api_key or f"nerilabs_sk_live_{secrets.token_hex(16)}"
        pk = publishable_key or f"nerilabs_pk_live_{secrets.token_hex(16)}"
        sec = token_signing_secret or secrets.token_hex(32)
        ...
        with conn:
            ...
            """, (
                tenant_id, organization_name, contact_email, auth0_sub, key, pk,
                sec, subscription_plan, "ACTIVE",
                stripe_customer_id, stripe_subscription_id, stripe_connect_account_id,
                1, 0, time.time()
            ))
