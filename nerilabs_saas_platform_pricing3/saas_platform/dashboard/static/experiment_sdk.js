...
// Event Ingestion Queue
    ...
    function handlePromotionalRescueAction(action, newToken) {
        ...
        // 2. Reduce the Price Live in the Page DOM
        if (action.discounted_price_formatted) {
            const priceEls = document.querySelectorAll("[data-neri-price], .price, .amount, #pro-price-amount, #pro-price");
            priceEls.forEach(el => {
                const oldPrice = el.getAttribute("data-neri-price") || el.innerText || "$79";
                el.innerHTML = `<span style="text-decoration: line-through; opacity: 0.55; font-size: 0.85em; margin-right: 6px;">${oldPrice}</span><span style="color: #3ECF8E; font-weight: 700;">${action.discounted_price_formatted}</span>`;
            });

            // Update CTA button to checkout at the discounted price
            const btnEls = document.querySelectorAll(".btn, .plan-cta, [data-stripe-price]");
            btnEls.forEach(btn => {
