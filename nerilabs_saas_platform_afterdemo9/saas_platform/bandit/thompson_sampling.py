...
class MABEngine:
    ...
    def select_variant_for_visitor(self, visitor_id: str) -> Tuple[Variant, str, float]:
        ...
        if under_explored:
            # During warm-up, explore non-control variants first so changes are immediately visible
            test_arms = [a for a in under_explored if not a["variant"].is_control]
            candidate_pool = test_arms if test_arms else under_explored
            chosen = min(candidate_pool, key=lambda a: a["impressions"])
