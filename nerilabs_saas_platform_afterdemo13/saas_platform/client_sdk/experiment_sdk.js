/**
 * NeriLabs Universal Autonomous Experimentation Client SDK (v3.1 Production)
...
(function() {
    ...
    const CONFIG = {
        ...
        timeoutMs: window.AI_EXPERIMENT_TIMEOUT_MS || 1200
    };
    ...
    function flushBeacon() {
        ...
        // Use fetch with keepalive: true (supports custom headers and body reliably)
        fetch(endpoint, {
            method: "POST",
            headers: {
        ...
        .then(res => {
            if (!res.ok) {
                console.debug("[NeriLabs Telemetry] Beacon response:", res.status);
            }
        })
        .catch(err => {
            console.debug("[NeriLabs Telemetry] Notice:", err);
    ...
    function applySingleAssignment(assignment, el) {
        ...
        if (assignment.is_control) {
            // Control (Baseline): Keep original unmodified DOM and styling
            console.info(`[NeriLabs Universal Engine] Visitor routed to Control (Baseline) for "${assignment.target_selector}"`);
        } else {
            // Variant: Apply AI synthesized copy, style, or pricing changes
            // 1. Copy mutation
        ...
        // Conversion tracking on this element (capture phase ensures tracking even if stopPropagation is used)
    ...
    function observeAndApply(assignment, maxWaitMs = 4000) {
        ...
        function tryApply() {
            ...
            if (el) {
                applied = true;
                applySingleAssignment(assignment, el);
                return true;
            }
            return false;
        }
