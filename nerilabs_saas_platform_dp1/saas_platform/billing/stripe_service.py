...
class StripeBillingService:
    ...
    def handle_saas_subscription_webhook(
        ...
        new_plan = "GROWTH" if "growth" in plan_id.lower() else ("SCALE" if "scale" in plan_id.lower() else "STARTER")
        existing_tenant = db.get_tenant_by_email(customer_email)
        if existing_tenant:
            existing_tenant.subscription_plan = new_plan
            existing_tenant.subscription_status = "ACTIVE"
            if stripe_cust_id:
                existing_tenant.stripe_customer_id = stripe_cust_id
            if stripe_sub_id:
                existing_tenant.stripe_subscription_id = stripe_sub_id
            db.update_tenant(existing_tenant)
    ...
    def create_pricing_test_variants(
        ...
        plans: List[Any],
        default_button_selector: Optional[str] = None
    ) -> List[Variant]:
        """
        Creates real Stripe Price IDs on the customer's connected Stripe account,
        uses founder-provided custom Stripe Price IDs / Payment Links, or generates deterministic IDs.
        """
        tenant = db.get_tenant(experiment.tenant_id)
        connect_account_id = tenant.stripe_connect_account_id if tenant else None
        custom_api_key = None
        if connect_account_id and (connect_account_id.startswith("rk_") or connect_account_id.startswith("sk_")):
            custom_api_key = connect_account_id
            connect_account_id = None

        variants: List[Variant] = []

        for p in plans:
            # Extract attributes whether p is tuple, dict, or PricingPlanInput
            if isinstance(p, tuple):
                name, amount_cents = p[0], p[1]
                stripe_price_id_input = None
                payment_link_input = None
                btn_sel = default_button_selector
            elif hasattr(p, "plan_name"):
                name = p.plan_name
                amount_cents = int(p.price_dollars * 100)
                stripe_price_id_input = getattr(p, "stripe_price_id", None)
                payment_link_input = getattr(p, "stripe_payment_link", None)
                btn_sel = getattr(p, "button_selector", None) or default_button_selector
            elif isinstance(p, dict):
                name = p.get("plan_name", p.get("name", "Plan"))
                amount_cents = int(float(p.get("price_dollars", p.get("amount_cents", 4900) / 100)) * 100)
                stripe_price_id_input = p.get("stripe_price_id")
                payment_link_input = p.get("stripe_payment_link")
                btn_sel = p.get("button_selector") or default_button_selector
            else:
                name = f"Plan {len(variants)+1}"
                amount_cents = 4900
                stripe_price_id_input = None
                payment_link_input = None
                btn_sel = default_button_selector

            stripe_price_id = stripe_price_id_input
            if not stripe_price_id:
                # 1. Attempt Stripe API Price creation on Connected Account or via custom API Key
            ...
            price_sel_val = (
                getattr(p, "price_selector", None) or 
                (p.get("price_selector") if isinstance(p, dict) else None) or 
            ...
            plan_def = PricingPlan(
                ...
                stripe_payment_link=payment_link_input,
                button_selector=btn_sel,
                price_selector=price_sel_val
            )
            ...
            variant = Variant(
                ...
                name=f"{name} (${amount_cents/100:.0f}/mo)",
                variant_type=VariantType.PRICING,
                generation_mode=experiment.generation_mode,
                hypothesis=f"Testing price elasticity point of ${amount_cents/100:.0f}/mo on subscription conversion.",
