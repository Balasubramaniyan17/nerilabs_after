...
def seed_production_experiment_if_empty():
    """Ensures that the live production experiment for nerilabs.io is always available with approved variants and MAB arms."""
    try:
        exp = db.get_experiment("exp_saas_hero_conversion")
        if not exp:
            exp = Experiment(
                experiment_id="exp_saas_hero_conversion",
                tenant_id="tenant_startup_01",
                title="NeriLabs Live Website Optimization",
                url="https://nerilabs.io",
                target_selector="a[href=\"#pricing\"], .hero-ctas .btn-primary, #navActionBtn",
                goal_event="CONVERSION",
                is_active=True
            )
            db.save_experiment(exp)

        existing_variants = db.list_variants_for_experiment("exp_saas_hero_conversion")
        if not existing_variants:
            raw_variants = AgentOrchestrator.generate_and_cache_experiment_suite(exp)
            mab = get_or_create_mab("exp_saas_hero_conversion", exp.tenant_id)
            ...
            print("[Startup] Seeded live production experiment exp_saas_hero_conversion with 10 variants.")
...
def startup_event():
    ...
    try:
        ...
        seed_production_experiment_if_empty()
...
def authenticate_tenant(x_api_key: Optional[str]) -> Tenant:
    ...
    if clean_key == Config.ADMIN_MASTER_KEY:
...
def auto_provision_starter_experiment_for_tenant(tenant_id: str, website_url: str = "https://nerilabs.io") -> Optional[Experiment]:
    """Auto-provisions an initial working experiment for any new tenant so their dashboard is immediately functional."""
    try:
        exps = db.list_experiments_for_tenant(tenant_id)
        if exps:
            return exps[0]

        exp_id = f"exp_{tenant_id[:12]}_hero"
        exp = Experiment(
            experiment_id=exp_id,
            tenant_id=tenant_id,
            title="Homepage Hero CTA Optimization",
            url=website_url,
            target_selector=".hero-ctas .btn-primary, #hero-cta, a[href=\"#pricing\"], .btn-primary",
            goal_event="CONVERSION",
            is_active=True
        )
        db.save_experiment(exp)
        ...
        print(f"[Provisioning] Auto-provision notice: {e}")
        return None
...
def sync_auth0_user(req: Auth0SyncReq):
    ...
    # Ensure tenant has an active starter experiment so dashboard is immediately functional
    auto_provision_starter_experiment_for_tenant(tenant.tenant_id)
...
def login_with_admin_key(req: AdminLoginReq):
    if req.admin_key.strip() != Config.ADMIN_MASTER_KEY:
...
def get_current_tenant(x_api_key: Optional[str] = Header(None, alias="X-API-Key")):
    ...
    res["is_admin"] = (tenant.tenant_id == "tenant_founder_admin" or tenant.api_key == Config.ADMIN_MASTER_KEY or tenant.subscription_plan == "ENTERPRISE")
    return res
...
def list_tenant_experiments(
    ...
    tenant = authenticate_tenant(x_api_key)
    if getattr(tenant, "is_admin", False) or tenant.tenant_id == "tenant_startup_01":
        exps = db.list_experiments_for_tenant("tenant_startup_01")
        if not exps:
            seed_production_experiment_if_empty()
            exps = db.list_experiments_for_tenant("tenant_startup_01")
        return exps

    exps = db.list_experiments_for_tenant(tenant.tenant_id)
    if not exps:
        starter = auto_provision_starter_experiment_for_tenant(tenant.tenant_id)
        if starter:
            exps = [starter]
    return exps
...
def create_pricing_experiment(
    ...
    tenant = authenticate_tenant(x_api_key)
    limits = get_plan_limits(tenant.subscription_plan)
    if not limits["has_stripe_pricing"]:
        raise HTTPException(
            status_code=403,
...
def synthesize_combinations(
    ...
    tenant = authenticate_tenant(x_api_key)
    limits = get_plan_limits(tenant.subscription_plan)
    if not limits["has_factorial"]:
        raise HTTPException(
            status_code=403,
            detail=f"Factorial combinations synthesis requires Scale plan ($199/mo) or Enterprise. Your current plan is {limits['name']}."
        )
    exp = db.get_experiment(experiment_id)
    if not exp or (exp.tenant_id != tenant.tenant_id and not getattr(tenant, 'is_admin', False) and tenant.subscription_plan != 'ENTERPRISE'):
        raise HTTPException(status_code=404, detail="Experiment not found")
