...
// Event Ingestion Queue
    ...
    function handlePromotionalRescueAction(action, newToken) {
        ...
        if (action.discounted_price_formatted) {
            const targetAssign = (activeAssignments && activeAssignments.length > 0) ? activeAssignments[0] : null;
            if (targetAssign) {
                const targetEls = resolveTargetElements(targetAssign);
                targetEls.forEach(el => {
                    const oldPrice = el.getAttribute("data-neri-price") || el.innerText || "$79";
                    el.innerHTML = `<span style="text-decoration: line-through; opacity: 0.5; font-size: 0.85em; margin-right: 6px;">${oldPrice}</span><span style="color: #3ECF8E; font-weight: 700;">${action.discounted_price_formatted}</span>`;
                ...
                try {
                    const btnEls = Array.from(document.querySelectorAll(btnSelector));
                    btnEls.forEach(btn => {
