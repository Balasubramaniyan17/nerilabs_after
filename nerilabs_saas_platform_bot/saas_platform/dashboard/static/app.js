...
let CURRENT_API_KEY = localStorage.getItem("nerilabs_api_key") || DEFAULT_API_KEY;
...
async function logoutUser() {
    localStorage.removeItem("nerilabs_api_key");
    localStorage.removeItem("nerilabs_exp_id");
    localStorage.removeItem("nerilabs_admin_bypass");
    CURRENT_API_KEY = "";
    CURRENT_TENANT = null;
    try {
        const client = await getAuth0Client();
...
async function initAuthentication() {
    ...
    if (stored) {
        try {
            ...
            if (params.get("upgraded") === "true" && params.get("plan")) {
                await fetch(`${API_BASE}/api/v1/billing/confirm-upgrade`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json", "X-API-Key": stored },
                    body: JSON.stringify({ plan: params.get("plan"), session_id: params.get("session_id") })
                });
                window.history.replaceState({}, document.title, window.location.pathname);
            }
...
function updateActiveExperimentBanner() {
    ...
    if (scriptBlock) {
        ...
        if (baseUrl.startsWith("http://") && !window.location.hostname.includes("localhost") && !window.location.hostname.includes("127.0.0.1")) {
    ...
    try {
        ...
        try {
            ...
            if (resOverall.ok) {
                ...
                // Update Bot & Traffic Source Metrics
                const botCountEl = document.getElementById("val-bot-count");
                if (botCountEl) botCountEl.textContent = (overall.bot_traffic_filtered || 0).toLocaleString();
...
async function loadVariantApprovalList() {
    ...
    if (!tbody || !CURRENT_EXP_ID) return;
    ...
    try {
        ...
        tbody.innerHTML = variants.map(v => {
            ...
            let statusBadge = '<span class="tag-badge tag-draft" style="color:#f59e0b; border-color:#f59e0b;">[ PENDING REVIEW ]</span>';
            if (status === "APPROVED" || status === "ACTIVE") {
                statusBadge = '<span class="tag-badge tag-approved" style="color:#3ECF8E; border-color:#3ECF8E;">[ APPROVED ]</span>';
            } else if (status === "REJECTED") {
                statusBadge = '<span class="tag-badge tag-rejected" style="color:#ef4444; border-color:#ef4444;">[ REJECTED ]</span>';
            }

            const copyPayload = v.copy_payload || {};
            const origText = copyPayload.original_text || "(Original Text)";
            const newText = copyPayload.new_text || (isControl ? origText : "Unspecified Text");
            const hypothesis = v.hypothesis || "Exploration arm.";
            const vType = v.variant_type || "COPY";

            let actionsHtml = '';
            if (isControl) {
                actionsHtml = '<span style="font-size: 11px; color: var(--ink-dim);">Baseline (Always Active)</span>';
            } else {
                const approveBtn = status !== "APPROVED" ? 
                    `<button class="btn btn-sm btn-primary" onclick="handleApproveVariant('${v.variant_id}')" style="padding: 3px 8px; font-size: 10.5px; background: rgba(62,207,142,0.15); color: #3ECF8E; border: 1px solid rgba(62,207,142,0.4);">✓ Approve</button>` : '';
                const rejectBtn = status !== "REJECTED" ? 
                    `<button class="btn btn-sm" onclick="handleRejectVariant('${v.variant_id}')" style="padding: 3px 8px; font-size: 10.5px; background: rgba(239,68,68,0.12); color: #ef4444; border: 1px solid rgba(239,68,68,0.3); margin-left: 4px;">✗ Deny</button>` : '';
                const editBtn = `<button class="btn btn-sm btn-secondary" onclick="handleEditVariant('${v.variant_id}', '${encodeURIComponent(newText)}')" style="padding: 3px 8px; font-size: 10.5px; margin-left: 4px;">✏️ Edit</button>`;
                actionsHtml = `<div style="display: flex; justify-content: flex-end; align-items: center; gap: 4px;">${approveBtn}${rejectBtn}${editBtn}</div>`;
            }

            return `
                <tr style="border-bottom: 1px solid var(--line);">
                    <td>${statusBadge}</td>
                    <td style="font-weight: 600; font-size: 12px; color: var(--ink);">${escapeHtml(v.name)}</td>
                    <td><code style="font-size: 10.5px; color: var(--ink-dim);">${vType}</code></td>
                    <td style="max-width: 280px; font-size: 12px;">
                        ${isControl ? `<span style="color: var(--ink-dim);">${escapeHtml(newText)}</span>` : 
                        `<div><span style="color: #3ECF8E; font-weight: 500;">"${escapeHtml(newText)}"</span></div>`}
                    </td>
                    <td style="max-width: 250px; font-size: 11px; color: var(--ink-dim); line-height: 1.35;">${escapeHtml(hypothesis)}</td>
...
async function handleEditVariant(variantId, currentTextEnc) {
    if (!CURRENT_EXP_ID) return;
    const currentText = decodeURIComponent(currentTextEnc);
    const updated = prompt("Edit the text that will be shown to visitors for this variant:", currentText);
    if (updated === null || updated.trim() === "" || updated.trim() === currentText) return;
    ...
    try {
        const res = await fetch(`${API_BASE}/api/v1/experiments/${CURRENT_EXP_ID}/variants/${variantId}`, {
            ...
            body: JSON.stringify({ copy_text: updated.trim() })
        });
        if (!res.ok) {
            const err = await res.json().catch(() => ({}));
...
// =========================================================================
// TRAFFIC QUALITY & SOURCE ATTRIBUTION
// =========================================================================
function renderTrafficSourceAttribution(sources) {
    ...
    if (!sources || Object.keys(sources).length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; color: var(--ink-dim); padding: 20px;">Awaiting live incoming visitors with UTM parameters or referral sources...</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = Object.entries(sources).map(([src, d]) => {
        ...
        const catBadge = isCasual ? 
            '<span style="font-size: 10.5px; padding: 2px 6px; background: rgba(245,158,11,0.12); color: #f59e0b; border: 1px solid rgba(245,158,11,0.3); border-radius: 3px;">Casual Social</span>' :
            '<span style="font-size: 10.5px; padding: 2px 6px; background: rgba(46,60,255,0.12); color: #2E3CFF; border: 1px solid rgba(46,60,255,0.3); border-radius: 3px;">High Intent</span>';

        return `
            <tr style="border-bottom: 1px solid var(--line);">
                <td style="font-weight: 600; color: var(--ink);">${escapeHtml(src)}</td>
                <td>${catBadge}</td>
                ...
                <td><span style="font-size: 10.5px; color: #3ECF8E;">Active Tracking</span></td>
            </tr>
        `;
    }).join("");
}
