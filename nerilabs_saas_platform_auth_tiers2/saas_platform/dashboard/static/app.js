...
async function loadTenantExperimentsList() {
    try {
        ...
        // Show all experiments; for founder or users, make sure live experiment is always available
        const activeList = (exps && exps.length > 0) ? exps : [];

        const sel = document.getElementById("header-experiment-select");
        if (sel && activeList.length > 0) {
            sel.innerHTML = "";
            ...
            // Automatically pick the live experiment
