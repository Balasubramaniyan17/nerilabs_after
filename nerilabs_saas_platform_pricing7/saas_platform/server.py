...
def create_pricing_experiment(
    ...
    # Deactivate prior experiments ONLY if they target the same pricing tier/selector
    target_sel = req.target_selector or "#pricing-table"
    for old_e in db.list_experiments_for_tenant(tenant.tenant_id):
        if old_e.experiment_type == ExperimentType.PRICING_TEST and old_e.experiment_id != req.experiment_id:
            if old_e.target_selector == target_sel:
                old_e.is_active = False
                db.save_experiment(old_e)
