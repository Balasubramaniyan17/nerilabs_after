...
// Event Ingestion Queue
    ...
    function setupBehavioralFrictionMonitors() {
        ...
        const dwellInterval = setInterval(() => {
            ...
            // Trigger when pricing hover dwell reaches 5s OR page dwell reaches 8s
            const shouldTrigger = (pricingHoverSeconds >= 5) || (dwellSeconds >= 8 && isHoveringPricing) || (dwellSeconds >= 12);
            const isPreviewOrTesting = previewParam || window.location.search.includes("preview") || window.location.search.includes("test_rescue");
            if (shouldTrigger && activeAssignments.length > 0 && !rescueModalRendered && (!sessionStorage.getItem("neri_rescue_shown") || isPreviewOrTesting)) {
                clearInterval(dwellInterval);
                console.info(`[NeriLabs Rescue] Dwell threshold reached (${pricingHoverSeconds}s pricing hover, ${dwellSeconds}s page dwell). Dispatching rescue request...`);
                queueEvent(
        ...
        document.addEventListener("mouseleave", function(e) {
            if (e.clientY <= 0 && activeAssignments.length > 0 && !rescueModalRendered && !sessionStorage.getItem("neri_rescue_shown")) {
