...
function selectInspectorVariant(variant, btnEl) {
    ...
    // Update rendered preview button
    ...
    if (previewContainer) {
        ...
        previewBtn.textContent = copyPayload.new_text || "Get Started Free";

        // Apply style
        const stylePayload = variant.style_payload || {};
        if (stylePayload.bg_color) previewBtn.style.backgroundColor = stylePayload.bg_color;
        if (stylePayload.text_color) previewBtn.style.color = stylePayload.text_color;
        if (stylePayload.border_radius) previewBtn.style.borderRadius = stylePayload.border_radius;
...
async function fetchAnalytics() {
    if (CURRENT_PREVIEW_VARIANTS.length === 0) renderTargetInspector();
    try {
        if (!CURRENT_EXP_ID) return;
...
async function handleOnboardingSubmit(e) {
    ...
    if (btn) {
        btn.disabled = true;
        btn.textContent = "[ 1/2 ] Synthesizing AI Variants...";
    }

    const urlEl = document.getElementById("ob-url");
    const selEl = document.getElementById("ob-selector");
    const textEl = document.getElementById("ob-text");
    const primEl = document.getElementById("ob-color-primary");
    const accEl = document.getElementById("ob-color-accent");

    const payload = {
        website_url: urlEl ? urlEl.value.trim() : "",
        target_selector: selEl ? selEl.value.trim() : "#primary-cta",
        target_element_text: textEl ? textEl.value.trim() : "Get Started Free",
        brand_primary_color: primEl ? primEl.value.trim() : "#2E3CFF",
        brand_accent_color: accEl ? accEl.value.trim() : "#3ECF8E"
    };

    try {
        if (btn) {
            setTimeout(() => {
                if (btn && btn.disabled) btn.textContent = "[ 2/2 ] Running Guardrails & Generating Script...";
            }, 700);
        }
        ...
        const mainScript = document.getElementById("main-script-tag-code");
        if (mainScript) mainScript.textContent = data.script_tag_html;

        if (window.loadTenantExperimentsList) await loadTenantExperimentsList();
        if (window.renderTargetInspector) renderTargetInspector();
        if (window.fetchAnalytics) fetchAnalytics();
...
async function scanMacroPageUrl(e) {
    ...
    try {
        ...
        const data = await res.json();
        const targets = data.targets || [];
        if (targets.length > 0 && selInput) {
            const first = targets.find(t => t.recommended) || targets[0];
            selInput.value = first.selector;
            selInput.style.borderColor = "var(--good)";
            setTimeout(() => { selInput.style.borderColor = "var(--line)"; }, 2000);
        }
    } catch (err) {
