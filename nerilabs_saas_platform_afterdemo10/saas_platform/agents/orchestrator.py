...
class AgentOrchestrator:
    ...
    def generate_and_cache_experiment_suite(cls, experiment: Experiment) -> List[Variant]:
        ...
        # 2 & 3. Generate Copy and UI Variants in Parallel (concurrent execution)
        import concurrent.futures
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            future_copy = executor.submit(CopyAgent.generate_variants, experiment)
            future_ui = executor.submit(UIAgent.generate_variants, experiment)
            try:
                copy_variants = future_copy.result(timeout=10)
            except Exception:
                copy_variants = CopyAgent._generate_fallback_variants(experiment, target_text)
            try:
                ui_variants = future_ui.result(timeout=10)
            except Exception:
                ui_variants = UIAgent._generate_fallback_variants(experiment)
